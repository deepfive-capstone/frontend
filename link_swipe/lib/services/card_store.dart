...
class CardStore {
  ...
  void add(VideoCardData card) {
    ...
    // DB의 status에 맞춰 초기 SwipeState 설정
    SwipeState initialState = SwipeState.none;
    if (card.status == 'liked') {
      initialState = SwipeState.liked;
    } else if (card.status == 'skipped') {
      initialState = SwipeState.skipped;
    }
    
    _cards.insert(0, StoredCard(data: card, state: initialState)); // 최신 카드가 맨 앞
  }
